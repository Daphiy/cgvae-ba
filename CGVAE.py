...
class DenseGGNNChemModel(ChemModel):
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions):
        ...
        local_graph_repr = tf.tile(local_graph_repr, [1, num_v, 1])  # [b, v, h+h]
        print('local_graph_repr shape', local_graph_repr.shape)
        ...
        print('combined_stop_node_repr',combined_stop_node_repr.shape)
    ...
    def construct_logit_matrices(self):
        ...
        __, cross_entropy_losses_final, edge_predictions_final = \
            tf.while_loop(
                lambda idx, cross_entropy_losses, edge_predictions : idx < self.placeholders[
                    'max_iteration_num'],
                self.generate_cross_entropy,
                (tf.constant(0), cross_entropy_losses, edge_predictions,))
    ...
    def construct_loss(self):
        ...
        # Edge loss
        self.ops["edge_loss"] = tf.reduce_sum(self.ops['cross_entropy_losses'] *
                                              self.placeholders['iteration_mask'], axis=1)
        # KL loss
    ...
    def calculate_incremental_results(self, raw_data, bucket_sizes, file_name):
        ...
        for idx, data_point in enumerate(raw_data):
            ...
            if not self.params["path_random_order"]:
                ...
                node_length = len(data_point["node_features"])
                if self.params["multi_bfs_path"]:
            ...
            for list_idx, starting_idx in enumerate(list_of_starting_idx):
                # choose a bucket
                chosen_bucket_idx = np.argmax(bucket_sizes > max([v for e in data_point['graph']
                                                                  for v in [e[0], e[2]]]))
    ...
    def process_raw_graphs(self, raw_data, is_training_data, file_name, bucket_sizes=None):
        ...
        for data_point, (incremental_adj_mat, distance_to_others, node_sequence, edge_type_masks, edge_type_labels, local_stop,
            ...
            bucketed[chosen_bucket_idx].append({
                ...
                'init': data_point["node_features"] + [[0 for _ in range(x_dim)] for __ in
                                              range(chosen_bucket_size - n_active_nodes)],
                'labels': [data_point["targets"][task_id][0] for task_id in self.params['task_ids']],
    ...
    def make_batch(self, elements, maximum_vertice_num):
        ...
        for data_point in elements:
            max_iteration_num = max(data_point['number_iteration'], max_iteration_num)
        ...
        for data_point in elements:
            ...
            batch_data['adj_mat'].append(data_point['adj_mat'])
            batch_data['init'].append(data_point['init'])
            batch_data['node_mask'].append(data_point['mask'])
            ...
            batch_data['edge_labels'].append(edge_labels +
                                             [np.zeros((maximum_vertice_num))
                                              for _ in range(max_iteration_num - data_point['number_iteration'])])
            batch_data['iteration_mask'].append([1 for _ in range(data_point['number_iteration'])] +
                                                [0 for _ in range(max_iteration_num - data_point['number_iteration'])])
            batch_data['local_stop'].append([int(s) for s in data_point["local_stop"]] +
                                            [0 for _ in range(max_iteration_num - data_point['number_iteration'])])

            target_task_values = []
            target_task_mask = []
            for target_val in data_point['labels']:
    ...
    def generate_graph_with_state(self, random_normal_states, num_vertices,
        ...
        for idx in starting_point:
            # generate a new molecule
            new_mol, total_log_prob = self.search_and_generate_molecule(idx, [],
                                                                        [], real_length,
