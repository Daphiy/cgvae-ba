...
def generate_label(ground_truth_graph, incremental_adj, node_in_focus, real_neighbor, real_n_vertices, params):
    ...
    for neighbor in range(real_n_vertices):
        ...
        if adjacent and not incre_adjacent:
            if not params["label_one_hot"]:
                edge_type_label.append((node_in_focus, neighbor, edge_type))
                edge_label.append((node_in_focus, neighbor))
            elif params["label_one_hot"] and neighbor == real_neighbor:
                edge_type_label.append((node_in_focus, neighbor, edge_type))
                edge_label.append((node_in_focus, neighbor))
...
def generate_incremental_adj(last_adj, node_in_focus, neighbor, edge_type):
...
def update_one_step(distance_to_others, node_sequence, node_in_focus, neighbor,
                    ...
                    local_stop_label, params, new_adj_mat, keep_prob):
    ...
    edge_type_mask, edge_mask = generate_mask(valences=valences,
                                              adj_mat=new_adj_mat,
                                              color=color,
                                              real_n_vertices=real_n_vertices,
    ...
    if not local_stop_label:
        ...
        edge_type_label, edge_label = generate_label(graph, new_adj_mat,
    ...
    distances = bfs_distance(node_in_focus, new_adj_mat)
    distances = [(start, node, params["truncate_distance"])
                 if d > params["truncate_distance"]
                 else (start, node, d) for
                 start, node, d in distances]

    distance_to_others.append(distances)
    # update the incremental adj mat at this step
    incremental_adj_mat.append(deepcopy(new_adj_mat))
...
def construct_incremental_graph(dataset, edges, max_n_vertices, real_n_vertices, node_symbol, params, initial_idx=0):
    ...
    while len(queue) > 0:
        ...
        for neighbor, edge_type in current_adj_list:
            ...
            if color[neighbor] < 2:
                update_one_step(distance_to_others=distance_to_others,
                                node_sequence=node_sequence,
                                node_in_focus=node_in_focus,
                                neighbor=neighbor,
                                edge_type_masks=edge_type_masks,
                                valences=valences,
                                incremental_adj_mat=incremental_adj_mat,
                                color=color,
                                real_n_vertices=real_n_vertices,
                                graph=graph,
                                ...
                                local_stop_label=False,
                                params=params,
                                new_adj_mat=up_to_date_adj_mat,
                                keep_prob=keep_prob)
                # Add the edge and obtain a new adj mat
                up_to_date_adj_mat = generate_incremental_adj(
